...
Good! Finally the length of "My" and "so" are the same, nothing to do. At the end of the process, the substrings s(i) verify:
...
For the first fixed tests the needed number of moves to get property (P) is given as a comment so that you can know 
if your process follows the rule.
*/

const strng = "who hit retaining The That a we taken"
const strng2= "way the my wall them him"
const shouldTrue = "sm big sm big saw"
/* 
Expected res:
...
for (let i = 0; i < arr.length -1; i++) {
  if (arr[i].length <= arr[i+1].length && isSmaller) {
    isSmaller = !isSmaller
    continue
  }
  if(arr[i].length >= arr[i+1].length && !isSmaller) {
